...
-- Telegram managers are existing active ERP users approved for the private
-- management bot. Telegram usernames are never used for authorization.
...
CREATE INDEX telegram_updates_queue_idx ON telegram_updates (next_attempt_at, received_at)
...
CREATE INDEX assistant_action_requests_pending_idx
    ON assistant_action_requests (app_user_id, expires_at) WHERE status = 'pending';
...
CREATE INDEX assistant_audit_log_user_created_idx ON assistant_audit_log (app_user_id, created_at DESC);
...
CREATE INDEX assistant_action_audit_user_created_idx ON assistant_action_audit (app_user_id, created_at DESC);
